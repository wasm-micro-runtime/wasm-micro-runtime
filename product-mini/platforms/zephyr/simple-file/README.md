...
module. [wasm-apps/file.c](./wasm-apps/file.c) creates a directory, writes a
...
* **WebAssembly Module**
    ...
    The Docker image ships the wasi-sdk in `/opt/wasi-sdk`
    (`$WASI_SDK_PATH`). Recompile the module after changing
    `wasm-apps/file.c`:

    1. **Compile:** in the `wasm-apps` folder.
        ```bash
        $WASI_SDK_PATH/bin/clang --sysroot=$WASI_SDK_PATH/share/wasi-sysroot \
          -Os -nodefaultlibs -lc -o file.wasm file.c \
          -z stack-size=8192 -Wl,--initial-memory=65536 \
          -Wl,--export=__heap_base -Wl,--export=__data_end
        ```
    2. **Generate the C header:** at the application root `simple-file/`.
        ```bash
        python3 to_c_header.py
        ```
        It writes `src/file.h`, which the Zephyr application embeds.
...
<inf> main: Wasm file size: 235412
<inf> main: main found
